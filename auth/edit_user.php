...
if (isset($_POST['submit'])) {
  ...
  $id         = intval($_POST['id']);  // user id from hidden field
  ...
  if (empty($id) || empty($username) || empty($email) || empty($role_id)) {
    ...
    // 1. Check duplicates (except current user)
    $dupCheckSql = "SELECT id FROM user WHERE (username = ? OR email = ?) AND id != ?";
    $checkStmt = $conn->prepare($dupCheckSql);
    ...
    if ($checkResult->num_rows > 0) {
      ...
      // 2. Get old password if password field is empty
      if (empty($password)) {
        $getPass = $conn->prepare("SELECT password FROM user WHERE id = ?");
        $getPass->bind_param("i", $id);
        $getPass->execute();
        $oldPass = $getPass->get_result()->fetch_assoc()['password'];
      ...
      $updateSql = "UPDATE user 
                          SET username = ?, email = ?, password = ?, role_id = ?, updated_at = ?
                          WHERE id = ?";
      ...
      if ($stmt->execute()) {
        ...
        $formError = "Failed to update user: " . $stmt->error;
      }
    }
  }
}
          ...
          <div class="card shadow-sm rounded-3">
            <div class="card-body">
              ...
              <form method="post" autocomplete="on">
                ...
                <!-- Hidden timestamps -->
                <input type="hidden" name="created_at" value="<?= $user['created_at'] ?? date('Y-m-d H:i:s') ?>">
